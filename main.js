...
const MAX_FILE_SIZE_MB = 49; // This will now be primarily enforced by Telegram's own limits
...
// --- Main Download Logic (FINAL WORKAROUND - NO SIZE CHECK) ---
async function startDownload(chatId, userId, videoUrl, format, isInline = false) {
    ...
    try {
        ...
        // --- FINAL WORKAROUND: Always send MP3s as an external link ---
        if (isMp3) {
            const downloadUrl = `https://cdn402.savetube.su/download?url=${encodeURIComponent(videoUrl)}&format=mp3`;
            const mp3Message = `✅ Your MP3 is ready! Click the button below to download externally.`;
            const externalLinkBtn = { inline_keyboard: [[{ text: `🔗 Download MP3`, url: downloadUrl }]] };

            if (isInline) {
                await sendTelegramMessage(chatId, mp3Message, { reply_markup: externalLinkBtn });
            } else {
                await editMessageText(mp3Message, { ...editTarget, reply_markup: externalLinkBtn });
            }
            // Increment download count even for external links
            await kv.atomic().sum(["users", userId, "downloads"], 1n).commit();
            return; 
        }

        // --- SIMPLIFIED Logic for Videos (No HEAD request) ---
        ...
        if (!fileRes.ok) {
             // If download fails, provide an external link as a fallback
            const errorMessage = `⚠️ Sorry, the download failed (${fileRes.status}). You can try downloading it directly.`;
            const externalLinkBtn = { inline_keyboard: [[{ text: `🔗 Download Externally`, url: downloadUrl }]] };
             if (isInline) await sendTelegramMessage(chatId, errorMessage, { reply_markup: externalLinkBtn });
            else await editMessageText(errorMessage, { ...editTarget, reply_markup: externalLinkBtn });
            return;
        }
        ...
        await sendMedia(chatId, fileBlob, 'video', `📥 Adiza-YT Bot`, fileName, info.title);
...
async function sendMedia(chatId, blob, type, caption, fileName, title) {
    ...
    // Check if Telegram rejected the file (e.g., too large)
    if (!response.ok) {
        const errorData = await response.json();
        console.error("Telegram sendMedia error:", errorData);
...
// --- Server Start ---
console.log("Starting Adiza Downloader Bot (v48 - No Size Check)...");
Deno.serve(handler);
